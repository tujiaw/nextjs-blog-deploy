...
tags: ['Web']
draft: false
summary: CSRF 是跨站域请求伪造攻击，2007 年被列为互联网 20 大安全隐患。文中介绍其背景、实例、攻击对象及三种防御策略（验证 Referer 字段、添加 token 并验证、自定义 HTTP 头属性并验证），并以 Java 代码示例，还探讨了防御方法选择，总结目前防御策略不完善，需重视并选择合适策略降低危害。 
---
